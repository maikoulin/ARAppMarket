...
public class PackageUtils {
    ...
    public static final String INSTALL_COMPLETED = "install_complete";
    public static final String INSTALL_ERROR = "install_error";

    public static void startInstallBroadcast(Context context, String appName) {

        Intent intent = new Intent(INSTALL);
        intent.putExtra("act", INSTALL_START);
        intent.putExtra("packageName", appName);
        context.sendBroadcast(intent);
    }

    public static void completedInstallBroadcast(Context context, String appName) {
        Intent intent = new Intent(INSTALL);
        intent.putExtra("act", INSTALL_COMPLETED);
        intent.putExtra("packageName", appName);
        context.sendBroadcast(intent);
    }

    public static void errorInstallBroadcast(Context context, String appName) {
        BasicDataInfo.updateStateBypackageName(VpnStoreApplication.mSQLiteDatabase, appName, DownloadManager.INSTALL_FAIL);
        Intent intent = new Intent(INSTALL);
        intent.putExtra("act", INSTALL_ERROR);
        intent.putExtra("packageName", appName);
        context.sendBroadcast(intent);
    }
     ...
     * @return
     */
    public static int install(Context context, String filePath, String appName1, String packageName, Handler handler) {

        startInstallBroadcast(context, packageName);
        int val = install(context, filePath);
        if (val == PackageUtils.INSTALL_NORMAL) {
            return PackageUtils.INSTALL_NORMAL;
        } else if (val != PackageUtils.INSTALL_SUCCEEDED) {

            if (handler != null) {
                handler.sendEmptyMessage(val);
            }

            errorInstallBroadcast(context, packageName);
            return PackageUtils.INSTALL_FAILED_INVALID_URI;
        }

        if (handler != null) {
            handler.sendEmptyMessage(PackageUtils.INSTALL_SUCCEEDED);
        }
        return PackageUtils.INSTALL_SUCCEEDED;
    }


    public static int install(Context context, String filePath, String packageName) {
    ...
    public static int install(Context context, String filePath) {
        if (isSystemApplication(context, context.getPackageName())) {
            LoggerUtil.d(TAG, "-isSystemApplication = true");
            return installSilent(context, filePath);
        }
        LoggerUtil.d(TAG, "-isSystemApplication = false");
        // 使用普通安装，可能会中间取消并不能按成功，失败算
        installNormal(context, filePath);

        return INSTALL_NORMAL;

    }

    /*
     *判断是否是系统应用，静默安装不需要再安装在system/app下才能判断的出来是否是系统应用
     */
    public static boolean isSystemAppUid(Context context) {
        ApplicationInfo info = context.getApplicationInfo();
        // 系统应用的uid是1000
        return 1000 == info.uid;
    }
    ...
    public static int installSilent(Context context, String filePath, String pmParams) {
         ...
         * /> in mainfest
         **/
//        StringBuilder command = new StringBuilder().append("export LD_LIBRARY_PATH=/vendor/lib:/system/lib \npm install ")
//                .append(pmParams == null ? "" : pmParams).append(" ").append(filePath.replace(" ", "\\ "));
        StringBuilder command = new StringBuilder().append("pm install ")
                .append(pmParams == null ? "" : pmParams).append(" ").append(filePath.replace(" ", "\\ "));

        CommandResult commandResult = ShellUtils.execCommand(command.toString(), false, true);
        ...
        LoggerUtil.d(TAG, filePath + " " + new StringBuilder().append("installSilent successMsg:").append(commandResult.successMsg)
                .append(", ErrorMsg:").append(commandResult.errorMsg).toString());
    ...
    public static String getFailReason(int failCode) {
        switch (failCode) {
...
//		}
//        if (PackageUtils.isSystemAppUid(context)) {
        return uninstallSilent(context, packageName);
//        }
...
//		CommandResult commandResult = ShellUtils.execCommand(command.toString(), !isSystemApplication(context), true);
    ...
    public static Boolean isTopActivity(Context context, String packageName) {
        ...
        if (null == tasksInfo || tasksInfo.isEmpty()) {
            return null;
        }
    ...
    public static final int INSTALL_NORMAL = 2;
